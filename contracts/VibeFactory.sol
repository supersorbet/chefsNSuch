...
contract VibeFactory is BoringBatchable {
    ...
    event ClaimantDropLimitedCreated(address indexed sender, address indexed proxy, bytes data);
    event VibeWhitelistSaleCreated(address indexed sender, address indexed proxy, bytes data);
    ...
    function createAirdrop(
        ...
        factory.exec(proxy, abi.encodeCall(Airdrop.setMerkleRoot, (merkleInformation.merkleRoot_, merkleInformation.externalURI_, merkleInformation.maxNonWhitelistedPerUser)), 0);
        factory.transferOwnership(proxy, msg.sender);
